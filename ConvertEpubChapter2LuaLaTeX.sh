...
#
# USAGE EXAMPLES:
# bash ConvertEpubChapter2LuaLaTeX.sh -i {epub-file-path} -o {output-zip-file-path} -p {pandoc-executable-path} -t {temp-dir-path}
# bash ConvertEpubChapter2LuaLaTeX.sh --in {epub-file-path} --out {output-zip-file-path} -p {pandoc-executable-path} -t {temp-dir-path}
...
echo '-------'RUN PANDOC
$PANDOC -s --from epub --to latex --variable documentclass=book --latex-engine=lualatex -o $TARGETFOLDER"/COMPLETE_"$TARGETFILENAME.tex $SOURCE
echo '-------'DONE
printf '%s\n'
...
for chapterxhtml in *.xhtml; do
    ...
    $PANDOC -s --from html --to latex --variable documentclass=article --latex-engine=lualatex -o $TARGETFOLDER"/Chapters/"$chaptername.tex $chapterxhtml
done
echo '-------'DONE
printf '%s\n'
