...
#
# USAGE EXAMPLES:
# bash ConvertEpubChapter2LaTeX.sh -i {epub-file-path} -o {output-zip-file-path} -p {pandoc-executable-path} -t {temp-dir-path}
# bash ConvertEpubChapter2LaTeX.sh --in {epub-file-path} --out {output-zip-file-path} -p {pandoc-executable-path} -t {temp-dir-path}
...
echo '-------'RUN PANDOC
$PANDOC -s --from epub --to latex --variable documentclass=book --latex-engine=pdflatex -o $TARGETFOLDER"/COMPLETE_"$TARGETFILENAME.tex $SOURCE
echo '-------'DONE
printf '%s\n'
...
for chapterxhtml in *.xhtml; do
    ...
    $PANDOC -s --from html --to latex --variable documentclass=article --latex-engine=pdflatex -o $TARGETFOLDER"/Chapters/"$chaptername.tex $chapterxhtml
done
echo '-------'DONE
printf '%s\n'
